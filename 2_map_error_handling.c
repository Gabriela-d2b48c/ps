...
/*   Updated: 2023/06/12 14:57:43 by gabriela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Check if all elements are valid (01PECG)
If so, return 0, if not, return 1 */
...
int	ft_check_map(t_struct *game)
{
	...
	if (ft_check_outer_elements(&game->gta) == 1)
	{
		ft_printf("Error! Walls are incomplete.\n");
		return (1);
	}
	if (ft_check_characters(&game->gta) == 1)
	{
